...
class WeatherViewModel {
  ...
  WeatherViewModel() {
    ...
    updateWeatherCommand = RxCommand.createAsync<String, List<WeatherEntry>>(
      update,
      canExecute: switchChangedCommand?.map((event) => event ?? false) as Stream<bool>?,
    ...
    textChangedCommand!.debounceTime(Duration(milliseconds: 500)).listen(
          (filterText) {
            ...
            updateWeatherCommand!.execute(filterText as String);
...
class WeatherEntry {
  ...
  WeatherEntry(City city) {
    ...
    this.iconURL =
        city.weather != null ? "http://openweathermap.org/img/w/${city.weather[0].icon}.png" : '';
    this.description = city.weather != null ? city.weather[0].description : '';
